...
class UniversitySpecialties extends BaseTableMonitoring
{
    /**
     * Наименование таблицы
     */
    private const TABLE_NAME = 'Список специальностей ВУЗов';

    /**
     * Массив названий полей таблицы
     */
    private const COLUMN_ARR = [
        '#', 'Аббревиатура', 'Полное название', 'Ссылка', 'Направление', 'Специальности'
    ];
    ...
    {
        return '
            SELECT `vuzes`.`abrev` as abrev, `vuzes`.`name` as name, `site`, `dir2specs`.`name` as dir_name, `specs`.`prof` as prof
            FROM `vuz`.`vuzes`
