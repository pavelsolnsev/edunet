...
class leads {
    ...
    static function show($v_id = 0) {
        ...
        $show_leads =  AuthHelper::canShowLeads($v_id, 'vuz');

        if ($show_leads) {
            $sql_text = '
            SELECT  SQL_CALC_FOUND_ROWS
              `al`.`id`, `al`.`service`, `al`.`name`, `al`.`phone`, `al`.`email`, `al`.`added`, `al`.`vuz_id`, `vv`.`abrev`
            FROM 
              `ads`.`leads` as al
            LEFT JOIN 
                `vuz`.`vuzes` as vv on vv.id = al.vuz_id 
            WHERE `al`.`service` = "vuz" AND (`al`.`vuz_id` = '.$v_id.' OR `vv`.`parent_id` = '.$v_id.') ORDER BY `id` DESC LIMIT '.$start.','.$rp;
